...
export class AdminDashboard implements OnInit {
  ...
  activeTab: 'courses' | 'tasks' | 'enrollment' | 'announcements' | 'teamGroups' | 'materials' = 'courses';
  ...
  // Unified form models
  materialForm = { id: undefined as number | undefined, title: '', url: '', course: { id: null as number | null } };
  newCourse = { code: '', name: '', credits: 3 };
  newOffering = { semesterId: null, course: { id: null } };
  newTask = { title: '', requirements: '', submissionUrl: '', dueDate: null, notes: '', offering: { id: null } };
  ...
  saveMaterial() {
    ...
    if (this.isEditingMaterial && this.materialForm.id) {
      this.materialService.updateMaterial(this.materialForm.id, this.materialForm as any).subscribe({
        next: () => {
  ...
  editMaterial(item: any) {
    ...
    this.materialForm = { id: item.id, title: item.title, url: item.url, course: { id: item.course.id } };
  ...
  saveCourse() { this.courseService.createCourse(this.newCourse as any).subscribe(() => this.refreshAllData()); }
  saveOffering() { this.offeringService.createOffering(this.newOffering as any).subscribe(() => this.refreshAllData()); }
  saveTask() { this.taskService.createTask(this.newTask as any).subscribe(() => alert('Task Published')); }
  saveAnnouncement() { this.announcementService.createAnnouncement(this.newAnnouncement as any).subscribe(() => alert('Announced')); }
  enrollStudent(studentId: number, offeringId: number) { this.http.post('http://localhost:8080/api/registrations', { studentId, offering: { id: offeringId }, status: 'REGISTERED' }, { withCredentials: true }).subscribe(() => this.checkEnrollmentStatus()); }
  checkEnrollmentStatus() { if (this.selectedOfferingId) this.http.get<number[]>(`http://localhost:8080/api/registrations/offering/${this.selectedOfferingId}`, { withCredentials: true }).subscribe(ids => this.enrolledStudentIds = ids); }
  saveTeamGroup() { this.teamGroupService.createGroup(this.newTeamGroup as any).subscribe(() => alert('Group Created')); }
}
